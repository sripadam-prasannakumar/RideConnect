...
const DriverDashboard = () => {
    ...
    // Polling for new rides (Alternative to WebSockets as requested)
    useEffect(() => {
        let pollInterval;
                            ...
                            if (!ignoredRequests.has(rId)) {
                                setAvailableRides(prev => {
                                    ...
                                    return [{
                                        ...
                                        duration_text: rData.duration_text || "Calculated soon",
                                        customer_name: rData.customer_name || "New Customer"
                                    }, ...prev];
                                });
                            }
                        } else if (event.channel === RIDE_UPDATES_CHANNEL && event.message.status === 'accepted') {
                            setAvailableRides(prev => prev.filter(r => r.id !== event.message.ride_id));
                        }
                    }
                });
    ...
    }, [isOnline, stats?.driver_id, userData.driver_id, userData.user_id, verificationStatus, ignoredRequests.size]);
            ...
            <div className="p-6 space-y-8 max-w-7xl mx-auto">
                ...
                {(verificationStatus === 'pending' || verificationStatus === 'rejected') && (
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
                        className={`p-4 rounded-xl border flex flex-col md:flex-row items-center justify-between gap-4 ${verificationStatus === 'rejected' ? 'bg-red-500/10 border-red-500/20 text-red-700 dark:text-red-400' : 'bg-primary/10 border-primary/20 text-primary-dark dark:text-primary'
                            }`}>
